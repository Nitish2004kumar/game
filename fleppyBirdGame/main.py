...
GAME_OVER = 'gallery/sprites/gameover.png'  # Path to the game over image
...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            ...
            # If the user presses space or up key, or clicks the mouse, start the game for them
...
def showGameOverScreen():
    ...
    pygame.display.update()
    pygame.time.wait(2000)  # Wait for 2 seconds before allowing the user to restart or exit
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                pygame.quit()
